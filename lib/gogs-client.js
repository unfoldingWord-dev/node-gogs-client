...
function API (apiUrl, requester) {
  ...
  _this.deleteUser = function (user, authUser) {
    if (user.username == authUser.username) {
      throw 'Users cannot delete themselves!';
    }
  ...
  _this.searchUsers = function (query, limit, authUser) {
    ...
    return request('users/search?q=' + query + '&limit=' + limit, authUser).then(stat.checkOkReponse);
  };
  ...
  _this.searchRepos = function (query, uid, limit) {
    ...
    return request('repos/search?q=' + query + '&uid=' + uid + '&limit=' + limit).then(stat.checkOkReponse);
  };
